...
set F_REDIST=yes
set F_DEPLOY=yes
if not "%CMD%" == "all" if not "%CMD%" == "dbapply" set F_DBAPPLY=no
if not "%CMD%" == "all" if not "%CMD%" == "redist" set F_REDIST=no
if not "%CMD%" == "all" if not "%CMD%" == "deployredist" set F_DEPLOY=no

if "%F_DBAPPLY%" == "yes" (
cd %ROOTPATH%\install\master\bin
call urm.cmd database dbapply %STDOPTS% -a -db maindb -dbpwd masterkey default all
if not %errorlevel% == 0 (
