...
public class ReleaseMaster {
	...
	public ReleaseMaster( Meta meta , ReleaseDist releaseDist ) {
		this.meta = meta;
	...
	public ReleaseMaster copy( Meta rmeta , ReleaseDist rrDist ) throws Exception {
		ReleaseMaster rMaster = new ReleaseMaster( rmeta , rrDist );
		
		for( ReleaseMasterHistory hItem : mapHistory.values() ) {
			ReleaseMasterHistory rhItem = hItem.copy( rMaster );
			rMaster.addHistoryItem( rhItem );
		}
		
		for( ReleaseMasterItem item : mapItem.values() ) {
			ReleaseMasterItem ritem = item.copy( rMaster );
