package ru.egov.urm.messenger.skype.wrapper;
...
*/
	 ...
	 * @since 4.0
	 * 
	 * @see com.skype.api.ParticipantListener#onLiveSessionVideosChanged(Participant object)
	 */
	...
	public void onMessage(com.skype.api.Conversation obj, Message message) {
		...
		if (msgType == Message.Type.POSTED_TEXT) {
            ...
            if (!msgAuthor.equals(mySession.myAccountName)) {
    			String msgChat = message.getConversation().getIdentity();
                String msgBody = message.getBodyXml();
                // Get timestamp -- it's in seconds, and the Date constructor needs milliseconds!
                Date dateTimeStamp = new Date((message.getTimestamp() * 1000L));
            	DateFormat targetDateFmt = DateFormat.getDateTimeInstance();
            	MySession.myConsole.printf("%s: [%s] chat id=%s - %s posted message%n%s%n",
            			mySession.myTutorialTag, targetDateFmt.format(dateTimeStamp), msgChat, msgAuthor, msgBody);
            }
        }
		else {
			/*
